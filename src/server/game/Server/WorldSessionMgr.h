...
private:
    ...
    void UpdateQueuedPlayers();
    bool CanAcceptQueuedSession(uint32 activeSessions, WorldSession* session) const;
    bool ShouldFakeQueue(WorldSession* session) const;
    uint32 GetFakeQueueDelay() const;
    ...
    Queue _queuedPlayer;
    std::unordered_map<uint32 /*accountId*/, Milliseconds> _fakeQueueReleaseTimes;
