...
namespace
{
    constexpr uint32 FAKE_QUEUE_MODE_DISABLED = 0;
    constexpr uint32 FAKE_QUEUE_MODE_ALWAYS = 1;
    constexpr uint32 FAKE_QUEUE_MODE_RANDOM = 2;
    constexpr uint32 MAX_FAKE_QUEUE_DELAY = 5 * MINUTE * IN_MILLISECONDS;
}
...
bool WorldSessionMgr::RemoveQueuedPlayer(WorldSession* session)
{
    ...
    for (; iter != _queuedPlayer.end(); ++iter, ++position)
    {
        if (*iter == session)
        {
            ...
            _fakeQueueReleaseTimes.erase(session->GetAccountId());
    ...
    if (!_queuedPlayer.empty() && CanAcceptQueuedSession(sessions, _queuedPlayer.front()))
    {
        WorldSession* pop_sess = _queuedPlayer.front();
        pop_sess->InitializeSession();
        pop_sess->SetInQueue(false);
        pop_sess->ResetTimeOutTime(false);
        _fakeQueueReleaseTimes.erase(pop_sess->GetAccountId());
...
void WorldSessionMgr::UpdateQueuedPlayers()
{
    if (_queuedPlayer.empty())
        return;
    ...
    while (iter != _queuedPlayer.end() && CanAcceptQueuedSession(activeSessions, *iter))
    {
        WorldSession* popSess = *iter;
        popSess->InitializeSession();
        popSess->SetInQueue(false);
        popSess->ResetTimeOutTime(false);
        _fakeQueueReleaseTimes.erase(popSess->GetAccountId());
...
void WorldSessionMgr::AddSession_(WorldSession* session)
{
    ...
    bool applyFakeQueueToSkipQueue = sWorld->getBoolConfig(CONFIG_FAKE_QUEUE_APPLY_TO_SKIP_QUEUE);
    if ((!canSkipQueue || applyFakeQueueToSkipQueue) && !HasRecentlyDisconnected(session) && ShouldFakeQueue(session))
    {
        _fakeQueueReleaseTimes[session->GetAccountId()] = GameTime::GetGameTimeMS() + Milliseconds(GetFakeQueueDelay());
        AddQueuedPlayer(session);
        UpdateMaxSessionCounters();
        return;
    }
...
bool WorldSessionMgr::CanAcceptQueuedSession(uint32 activeSessions, WorldSession* session) const
{
    if (GetPlayerAmountLimit() && activeSessions >= GetPlayerAmountLimit())
        return false;

    auto fakeQueueReleaseTime = _fakeQueueReleaseTimes.find(session->GetAccountId());
    return fakeQueueReleaseTime == _fakeQueueReleaseTimes.end() || GameTime::GetGameTimeMS() >= fakeQueueReleaseTime->second;
}

bool WorldSessionMgr::ShouldFakeQueue(WorldSession* /*session*/) const
{
    uint32 const mode = sWorld->getIntConfig(CONFIG_FAKE_QUEUE_MODE);

    if (mode == FAKE_QUEUE_MODE_DISABLED)
        return false;

    if (GetFakeQueueDelay() == 0)
        return false;

    if (mode == FAKE_QUEUE_MODE_ALWAYS)
        return true;

    if (mode == FAKE_QUEUE_MODE_RANDOM)
        return roll_chance_i(std::min(sWorld->getIntConfig(CONFIG_FAKE_QUEUE_CHANCE), uint32(100)));

    return false;
}

uint32 WorldSessionMgr::GetFakeQueueDelay() const
{
    return std::min(sWorld->getIntConfig(CONFIG_FAKE_QUEUE_DELAY), MAX_FAKE_QUEUE_DELAY);
}
