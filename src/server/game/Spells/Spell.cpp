...
namespace
{
bool IsCasterInTargetFrontForBehindCheck(Unit const* target, Unit const* caster)
{
    ...
    if (target->HasUnitState(UNIT_STATE_CANNOT_AUTOATTACK | UNIT_STATE_DISTRACTED))
        return false;

    float const victimAngle = target->GetAbsoluteAngle(victim);
    float casterAngle = Position::NormalizeOrientation(target->GetAbsoluteAngle(caster) - victimAngle);
    if (casterAngle > M_PI)
        casterAngle -= 2.0f * M_PI;

    return std::fabs(casterAngle) <= M_PI / 2.0f;
}
}
...
SpellCastResult Spell::CheckCast(bool strict, uint32* /*param1*/, uint32* /*param2*/)
{
    ...
    if (Unit* target = m_targets.GetUnitTarget())
    {
        ...
        if (target != m_caster)
        {
            ...
            if (m_spellInfo->HasAttribute(SPELL_ATTR0_CU_REQ_CASTER_BEHIND_TARGET) && IsCasterInTargetFrontForBehindCheck(target, m_caster))
...
SpellCastResult Spell::CheckRange(bool strict)
{
    ...
    if (target)
    {
        if (target != m_caster)
        {
            ...
            if (m_caster->IsPlayer() && m_spellInfo->HasAttribute(SPELL_ATTR0_CU_REQ_CASTER_BEHIND_TARGET) && IsCasterInTargetFrontForBehindCheck(target, m_caster))
                return SPELL_FAILED_NOT_BEHIND;
