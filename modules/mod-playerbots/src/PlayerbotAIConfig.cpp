...
namespace
{
constexpr uint32 PROGRESSION_PRE_TBC_STAGE = 8;
constexpr uint32 PROGRESSION_WOTLK_STAGE = 13;
constexpr uint32 ZONE_EVERSONG_WOODS = 3430;
constexpr uint32 ZONE_GHOSTLANDS = 3433;
constexpr uint32 ZONE_AZUREMYST_ISLE = 3524;
constexpr uint32 ZONE_BLOODMYST_ISLE = 3525;

uint32 GetProgressionMaxLevel(uint32 progressionStage)
{
    if (progressionStage >= PROGRESSION_WOTLK_STAGE)
        return 80;

    if (progressionStage >= PROGRESSION_PRE_TBC_STAGE)
        return 70;

    return 60;
}

bool IsTbcStartingZone(uint32 zoneId)
{
    return zoneId == ZONE_EVERSONG_WOODS ||
           zoneId == ZONE_GHOSTLANDS ||
           zoneId == ZONE_AZUREMYST_ISLE ||
           zoneId == ZONE_BLOODMYST_ISLE;
}

bool IsExpansionMapAllowedByLevel(uint32 mapId, uint32 maxLevel, bool allowTbcStartingZones)
{
    if (mapId == MAP_OUTLAND)
    {
        if (allowTbcStartingZones)
            return true;
...
bool PlayerbotAIConfig::Initialize()
{
    ...
    progressiveRandomBots = sConfigMgr->GetOption<bool>("AiPlayerbot.ProgressiveRandomBots", true);
    progressiveRandomBotsRestrictRaces =
        sConfigMgr->GetOption<bool>("AiPlayerbot.ProgressiveRandomBotsRestrictRaces", false);
    progressiveRandomBotsRestrictClasses =
        sConfigMgr->GetOption<bool>("AiPlayerbot.ProgressiveRandomBotsRestrictClasses", false);
    randomBotProgressionStage = 0;
    randomBotProgressionMaxLevel = sWorld->getIntConfig(CONFIG_MAX_PLAYER_LEVEL);

    bool const individualProgressionConfigured =
        !sConfigMgr->GetKeysByString("IndividualProgression.Enable").empty();
    bool const individualProgressionEnabled =
        individualProgressionConfigured &&
        sConfigMgr->GetOption<bool>("IndividualProgression.Enable", true, false);
    if (progressiveRandomBots && individualProgressionEnabled)
    {
        uint32 progressionLimit = sConfigMgr->GetOption<uint32>("IndividualProgression.ProgressionLimit", 0, false);
        uint32 startingProgression =
            sConfigMgr->GetOption<uint32>("IndividualProgression.StartingProgression", 0, false);
        randomBotProgressionStage = progressionLimit ? progressionLimit : startingProgression;
        randomBotProgressionMaxLevel = GetProgressionMaxLevel(randomBotProgressionStage);
    }
    ...
    if (progressiveRandomBots && randomBotMaxLevel > randomBotProgressionMaxLevel)
        randomBotMaxLevel = randomBotProgressionMaxLevel;
    if (randomBotMinLevel > randomBotMaxLevel)
        randomBotMinLevel = randomBotMaxLevel;

    effectiveRandomBotMaps.clear();
    for (uint32 mapId : randomBotMaps)
    {
        bool const allowTbcStartingZones = !progressiveRandomBotsRestrictRaces;
        if (!progressiveRandomBots ||
            IsExpansionMapAllowedByLevel(mapId, randomBotProgressionMaxLevel, allowTbcStartingZones))
        {
            effectiveRandomBotMaps.push_back(mapId);
        }
    }

    if (effectiveRandomBotMaps.empty())
    {
        if (progressiveRandomBots)
        {
            effectiveRandomBotMaps.push_back(0);
            effectiveRandomBotMaps.push_back(1);
        }
        else
        {
            effectiveRandomBotMaps = randomBotMaps;
        }
    }

    effectiveRandomBotMapsAsString.clear();
    for (uint32 mapId : effectiveRandomBotMaps)
    {
        if (!effectiveRandomBotMapsAsString.empty())
            effectiveRandomBotMapsAsString += ",";

        effectiveRandomBotMapsAsString += std::to_string(mapId);
    }

    if (progressiveRandomBots)
    {
        LOG_INFO("server.loading",
                 "Playerbot progression: stage {}, max random bot level {}, random bot maps {}",
                 randomBotProgressionStage, randomBotMaxLevel, effectiveRandomBotMapsAsString);
    }
...
bool PlayerbotAIConfig::IsRandomBotRaceAllowed(uint8 race) const
{
    if (!progressiveRandomBots || !progressiveRandomBotsRestrictRaces)
        return true;

    if (randomBotProgressionMaxLevel < 70 && (race == RACE_BLOODELF || race == RACE_DRAENEI))
        return false;

    return true;
}

bool PlayerbotAIConfig::IsRandomBotClassAllowed(uint8 cls) const
{
    if (!progressiveRandomBots || !progressiveRandomBotsRestrictClasses)
        return true;

    if (randomBotProgressionMaxLevel < 80 && cls == CLASS_DEATH_KNIGHT)
        return false;

    return true;
}

bool PlayerbotAIConfig::IsRandomBotZoneAllowed(uint32 mapId, uint32 zoneId) const
{
    if (!progressiveRandomBots)
        return true;

    if (randomBotProgressionMaxLevel < 70 && mapId == MAP_OUTLAND)
        return !progressiveRandomBotsRestrictRaces && IsTbcStartingZone(zoneId);

    if (randomBotProgressionMaxLevel < 80 && mapId == MAP_NORTHREND)
        return false;
