...
bool PlayerbotAI::CheckLocationDistanceByLevel(Player* player, const WorldLocation& loc, bool fromStartUp)
{
    ...
    {
        uint32 zoneId = map->GetZoneId(player->GetPhaseMask(), loc.GetPositionX(), loc.GetPositionY(),
                                       loc.GetPositionZ());
        auto const bracket = sRandomPlayerbotMgr.zone2LevelBracket.find(zoneId);
        if (bracket != sRandomPlayerbotMgr.zone2LevelBracket.end() && player->GetLevel() > bracket->second.high)
            return false;
    }
