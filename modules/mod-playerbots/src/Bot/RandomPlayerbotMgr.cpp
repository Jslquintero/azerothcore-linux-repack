    ...
    if (currentBots.size() < maxAllowedBotCount)
    {
        ...
        auto tryLoginBot = [&](const CharacterInfo& charInfo) -> bool
        {
            if (GetEventValue(charInfo.guid, "add") ||
                ...
                !sPlayerbotAIConfig.IsRandomBotClassAllowed(charInfo.rClass) ||
                !sPlayerbotAIConfig.IsRandomBotRaceAllowed(charInfo.rRace))
            {
                return false;
            }
...
void RandomPlayerbotMgr::RandomTeleport(Player* bot, std::vector<WorldLocation>& locs, bool hearth)
{
    ...
    tlocs.erase(std::remove_if(tlocs.begin(), tlocs.end(),
                               [bot](WorldPosition l)
                               {
                                   std::vector<uint32>::iterator i =
                                       find(sPlayerbotAIConfig.effectiveRandomBotMaps.begin(),
                                            sPlayerbotAIConfig.effectiveRandomBotMaps.end(), l.getMapId());
                                   return i == sPlayerbotAIConfig.effectiveRandomBotMaps.end();
                               }),
                tlocs.end());
    if (tlocs.empty())
    {
    ...
    for (uint32 i = 0; i < tlocs.size(); i++)
    {
        ...
        if (!sPlayerbotAIConfig.IsRandomBotZoneAllowed(map->GetId(), zone->ID))
            continue;
...
void RandomPlayerbotMgr::PrepareTeleportCache()
{
    ...
    QueryResult results = WorldDatabase.Query(
        ...
        "ORDER BY "
        "t.minlevel;",
        sPlayerbotAIConfig.effectiveRandomBotMapsAsString.c_str());
...
void RandomPlayerbotMgr::RandomizeFirst(Player* bot)
{
    ...
    {
        uint32 fixedLevel = bot->getClass() == CLASS_DEATH_KNIGHT
                                ? std::max(sPlayerbotAIConfig.randombotStartingLevel,
                                           sWorld->getIntConfig(CONFIG_START_HEROIC_PLAYER_LEVEL))
                                : sPlayerbotAIConfig.randombotStartingLevel;
        level = std::min(fixedLevel, maxLevel);
    }
