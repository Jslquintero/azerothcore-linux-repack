...
Player* RandomPlayerbotFactory::CreateRandomBot(WorldSession* session, uint8 cls, std::unordered_map<NameRaceAndGender, std::vector<std::string>>& nameCache)
{
    ...
    if (!sPlayerbotAIConfig.IsRandomBotClassAllowed(cls))
    {
        LOG_DEBUG("playerbots", "Skipping random bot class {} because it is not available at progression stage {}",
                  cls, sPlayerbotAIConfig.randomBotProgressionStage);
        return nullptr;
    }
    ...
    for (uint8 race = RACE_HUMAN; race < sRaceMgr->GetMaxRaces(); ++race)
    {
        ...
        if (!sPlayerbotAIConfig.IsRandomBotRaceAllowed(race))
            continue;
