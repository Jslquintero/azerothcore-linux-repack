...
namespace
{
void HandlePushQuestToParty(Player* bot, uint32 questId)
{
    WorldPacket packet(CMSG_PUSHQUESTTOPARTY, 4);
    packet << questId;
    WorldPackets::Quest::PushQuestToParty packetWrapper(std::move(packet));
    packetWrapper.Read();
    bot->GetSession()->HandlePushQuestToParty(packetWrapper);
}
}
...
bool ShareQuestAction::Execute(Event event)
{
    ...
    for (uint8 slot = 0; slot < MAX_QUEST_LOG_SIZE; ++slot)
    {
        ...
        if (logQuest == entry)
        {
            HandlePushQuestToParty(bot, entry);
...
bool AutoShareQuestAction::Execute(Event event)
{
    ...
    for (uint8 slot = 0; slot < MAX_QUEST_LOG_SIZE; ++slot)
    {
        ...
        HandlePushQuestToParty(bot, logQuest);
        botAI->TellMaster("Quest shared");
        shared = true;
    }
