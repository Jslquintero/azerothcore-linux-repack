...
namespace
{
void HandleQuestLogRemoveQuest(WorldSession* session, uint8 slot)
{
    WorldPacket worldPacket(CMSG_QUESTLOG_REMOVE_QUEST, 1);
    worldPacket << slot;
    WorldPackets::Quest::QuestLogRemoveQuest packet(std::move(worldPacket));
    packet.Read();
...
bool NewRpgBaseAction::OrganizeQuestLog()
{
    ...
    for (uint16 i = 0; i < MAX_QUEST_LOG_SIZE; ++i)
    {
            ...
            bot->GetQuestStatus(questId) == QUEST_STATUS_FAILED)
        {
            ...
            HandleQuestLogRemoveQuest(bot->GetSession(), (uint8)i);
            if (botAI->GetMaster())
